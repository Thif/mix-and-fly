...
if "favorites" in st.session_state:
    num_favorites = len(st.session_state.favorites) 
    st.markdown(f"##### 🌟 You have **{num_favorites}** favorite blend(s) !")

    st.dataframe(pd.DataFrame(st.session_state.favorites),hide_index=True)
else:
    st.markdown(f"##### 🌟 You haven't added favorite blends from the design tool yet !")